...
export default function SellerChatPage() {
  ...
  return (
    <div className="animate-fade-in">
      <h1 className="text-2xl font-bold mb-6">Chat Pembeli</h1>
      ...
      <div className="flex h-[calc(100vh-200px)] min-h-[500px] bg-white rounded-lg shadow-sm border border-outline-variant/30 overflow-hidden">
        ...
        <div className="w-full md:w-1/3 border-r border-outline-variant/30 flex flex-col">
          ...
          <div className="overflow-y-auto flex-grow">
            {rooms.length === 0 ? (
              <div className="p-8 text-center text-outline text-sm">Belum ada obrolan</div>
            ) : rooms.map(room => {
              const partner = room.buyer || room.store // For generic reuse, but here partner is buyer
          ...
          <div className="hidden md:flex w-2/3 flex-col bg-surface-container-lowest">
            <div className="p-4 border-b border-outline-variant/30 flex items-center justify-between bg-white shadow-sm z-10">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full ${!(activeRoomData.buyer?.imageUrl || activeRoomData.store?.imageUrl) ? getAvatarColor((activeRoomData.buyer || activeRoomData.store).name) : 'bg-surface-container'} text-white flex items-center justify-center font-bold text-lg shrink-0 shadow-sm overflow-hidden`}>
                  {(activeRoomData.buyer?.imageUrl || activeRoomData.store?.imageUrl) ? (
                    <img src={activeRoomData.buyer?.imageUrl || activeRoomData.store?.imageUrl} alt={(activeRoomData.buyer || activeRoomData.store).name} className="w-full h-full object-cover" />
                  ) : (
                    (activeRoomData.buyer || activeRoomData.store).name.charAt(0).toUpperCase()
                  )}
                </div>
                <div>
                  <div className="font-bold text-sm text-on-surface">{activeRoomData.buyer.name}</div>
                </div>
              </div>
            </div>
